...
def effective_bandwidth_gbps(rows: int, cols: int, ms: float) -> float:
    if ms <= 0:
        return 0.0
    n = rows * cols
    # Approximate traffic for this simple row-wise softmax implementation:
    # read input for max, read input/write exp output for sum, then read/write
    # output for normalization. This ignores the high cost of exp itself.
    bytes_touched = 5 * n * 4
...
def main() -> None:
    ...
    for rows, cols in shapes:
        ...
        cpu_ms = float(raif.benchmark_softmax_cpu(rows, cols, cpu_iters, 5))
        ...
        speedup = None
        if raif.cuda_available():
            cuda_ms = float(raif.benchmark_softmax_cuda(rows, cols, cuda_iters, 20))
    ...
    csv_path = results_dir / "softmax_benchmark.csv"
    with csv_path.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows_out[0].keys()))
        writer.writeheader()
        writer.writerows(rows_out)

    md_path = results_dir / "softmax_benchmark.md"
    with md_path.open("w") as f:
        f.write("# Softmax Benchmark Results\n\n")
